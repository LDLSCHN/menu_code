...
void Key_Run(void)
{
    static uint8 key2_state,key3_state,key4_state = 0;
    key2_state = my_key_get_state(MY_KEY_2);
    key3_state = my_key_get_state(MY_KEY_3);
    key4_state = my_key_get_state(MY_KEY_4);
    ...
    {
        Key_Timer_ShortRing();
        if (Steer_status == status_steer_stop)
        {
            Steer_activate();
        }
        else
